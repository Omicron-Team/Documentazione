...
\begin{itemize}
\item \textit{Java}\ped{G} (versione 8+), \textit{Python}\ped{G} o \textit{Node.js}\ped{G} per lo sviluppo del server back-end\ped{G};
\item Protocolli asincroni\ped{G} per le comunicazioni dell'applicazione mobile-server;
\item \textit{Blockchain}\ped{G}: sistema per salvare in modo immutabile e certificato i dati relativi alle sanificazioni:
\item \textit{Kubernetes}\ped{G}, \textit{Openshift}\ped{G} o \textit{Rancher}\ped{G} per il rilascio delle componenti
del server nonché per la gestione della scalabilità orizzontale\ped{G};
\item API\ped{G} \textit{REST}\ped{G} attraverso le quali sia possibile utilizzare l'applicativo;
\item \textit{Java}\ped{G} (attraverso il kit di sviluppo software Android\ped{G}) o \textit{Swift}\ped{G} per lo sviluppo dell'applicazione mobile rispettivamente Android\ped{G} o iOS\ped{G}.
\end{itemize}
}
...
\item Utilizzo dei \textit{tag RFID}\ped{G} per il tracciamento in tempo reale;
...
\item Vengono messi a disposizione dall'azienda dei server dedicati per effettuare test sui nostri componenti applicativi sviluppati, con relativa spiegazione sull'uso di \textit{Docker}\ped{G} (attraverso un seminario tecnologico).
\end{itemize}
}
