...
Il capitolato richiede per gran parte l'utilizzo di tecnologie fornite da \textit{Amazon Web Services} (AWS)\ped{G}, in particolare viene consigliato di utilizzare:
\begin{itemize}
\item \textit{Amazon API Gateway}: framework\ped{G} per la gestione di eventi HTTP\ped{G};
\item \textit{DynamoDB}: database NoSQL\ped{G} scalabile e distribuito;
...
\begin{itemize}
\item \textit{Typescript}\ped{G} come linguaggio di programmazione principale;
\item \textit{Next.js}\ped{G} come framework\ped{G} per l'implementazione del front-end\ped{G};
\item \textit{Serverless}\ped{G} come framework\ped{G} per l'implementazione del back-end\ped{G};
\item \textit{CloudWatch}\ped{G} o \textit{DataDog}\ped{G} come sistema di monitoraggio della piattaforma.
\end{itemize}
}
...
\item \textit{Stripe}\ped{G} come fornitore di servizi di pagamento;
\item Utilizzo dell'ultima versione di Typescript\ped{G}, dell'approccio centralizzato sfruttando i meccanismi \textit{Promise/Async-Await} (funzionalità specifiche fornite dal linguaggio stesso) e del gestore per la standardizzazione del codice \textit{ESlint}\ped{G};
\item \textit{GitHub}\ped{G} per la pubblicazione e il versionamento del codice.
\end{itemize}
}
