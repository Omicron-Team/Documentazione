...
Secret & NEXT\_PUBLIC\_API\_ID & Production & API endpoint provided by AWS when the backend is deployed to the staging environment (or the domain, if used)\\
Plaintext & NEXT\_PUBLIC\_API\_ID & Preview & API endpoint provided by AWS when the backend is deployed to the test environment (or the domain, if used)\\
Plaintext & NEXT\_PUBLIC\_API\_ID & Development & API endpoint provided by AWS when he backend is deployed to the local environment (or the domain, if used)\\
