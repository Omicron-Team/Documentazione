...
def append_to_letter_file(termine,definizione):
    ...
    with open("DocumentazioneEsterna/Glossario/res/Alfabeto/"+termine[0].upper()+".tex", "a") as letter_file:
       letter_file.write(latex_code)

def main():
    clean_file()
    definizioni = sorted(csv.DictReader(open('DocumentazioneEsterna/Glossario/glossario.csv')), key=lambda termine: termine['Termine'])
